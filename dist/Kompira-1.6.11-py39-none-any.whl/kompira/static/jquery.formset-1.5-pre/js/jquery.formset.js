    ...
    $.fn.formset = function(opts)
    {
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            updateElementIndex = function(elem, prefix, ndx) {
                // 下記パターンマッチにおいて、<attr> は ScheduleParameterFormなど inlineformset_factory で
                // 処理されるフィールド名に対応している。
                let idRegex = new RegExp(prefix + '-(\\d+|__prefix__)-((?<attr>\\w+)|(?<key>.*)(?<suffix>_\\d+))?(-(file|clear)(_id)?)?$');
                for (attr of ['for', 'id', 'name']) {
                    let val = elem.attr(attr);
                    if (val) {
                        let m = val.match(idRegex);
                        let replacement = `${prefix}-${ndx}-${m.groups.attr||m.groups.suffix||''}`;
                        elem.attr(attr, val.replace(idRegex, replacement));
                    }
                }
            },
            ...
            insertDeleteLink = function(row) {
                ...
                row.find('a.' + delCssSelector).click(function() {
                    ...
                    if (del.length) {
                        ...
                        //
                        // 要素削除 -> 追加した時に、インデックスがかぶらないようにコメントアウト
                        //
        ...
        $$.each(function(i) {
            ...
            if (hasChildElements(row)) {
                ...
                // 修正 by Fixpoint
                // ----------------
                // テーブルに関連付けられたフォームの場合、テーブルページロード時はフォームダイアログが hidden 状態のために
                // 削除リンクが表示されない問題を回避するため visible 条件を外す。
                //
                //if (row.is(':visible')) {
